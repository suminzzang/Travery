          ...
          <b-row class="mb-2 d-flex justify-content-between">
            <b-col cols="auto">
              <b-button class="w-auto"  id="btn-mv-register">
                <router-link v-if="isAdmin" :to="{ name: 'adminboardwrite' }" style="color: white;">글쓰기</router-link>
                <div v-else style="color: white;" @click="mvwrite">글쓰기</div>
              </b-button>
            </b-col>
            <b-col cols="auto" class='d-flex'>
              <b-form class="d-flex align-items-center" id="form-search" @submit.prevent>
                <b-form-select
                  :options="option"
                ...
                <b-button id="btn-search" class="w-auto text-nowrap" @click="search" @keypress.enter="search">검색</b-button>
              </b-form>
            </b-col>
          </b-row>
        </b-col>
