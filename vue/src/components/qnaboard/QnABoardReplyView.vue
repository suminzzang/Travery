          ...
          <b-form>
            ...
            <b-col cols='auto' class="text-center">
              <b-button  class="btn btn-outline-primary mb-3" style="background-color: white;">
                <router-link :to="{ name: 'qnaboardreplywrite',params: { articleNo:`${this.parent}` }}">답변</router-link>
              </b-button>
              ...
              <b-button v-if="isWriter" class="btn btn-outline-primary mb-3" style="background-color: white;">
                <router-link v-if="isWriter" :to="{ name: 'qnaboardreplymodify',params: { articleNo:`${this.articleNo}` }}">글수정</router-link>
              </b-button>
              <span v-if="isWriter" style="margin: 0 10px;"></span> <!-- 빈 칸 -->
...
export default {
  ...
  methods: {
    deleteBoard() {
      http.delete(`/qnaboardreply/${this.articleNo}`)
      .then(() => {
  ...
  created() {
    http.get(`/qnaboardreply/${this.articleNo}`)
      ...
      this.content = response.data.content;
        this.parent = response.data.parentNo;
