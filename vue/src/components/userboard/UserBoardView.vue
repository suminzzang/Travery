          ...
          <b-form>
            ...
            <b-col cols='auto' class="text-center">
              <b-button v-if="isWriter" class="btn btn-outline-primary mb-3" style="background-color: white;">
                <router-link v-if="isWriter" :to="{ name: 'userboardmodify',params: { articleNo:`${this.articleNo}` }}">글수정</router-link>
              </b-button>
...
export default {
  ...
  methods: {
    deleteBoard() {
      http.delete(`/userboard/${this.articleNo}`)
      .then(() => {
  ...
  created() {
    http.get(`/userboard/${this.articleNo}`)
      .then((response) => {
